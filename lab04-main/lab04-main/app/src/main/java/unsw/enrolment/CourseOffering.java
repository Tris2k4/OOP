...
public class CourseOffering {
    ...
    private boolean checkValidEnrolment(Student student) {
        ...
        for (Course prereq : prereqs) {
            ...
            boolean valid = studentEnrolments.stream().anyMatch(enrolment -> enrolment.enrolCheck(prereq));
            if (!valid) {
                return false;
            }
        }
    ...
    public List<Student> studentsEnrolledInCourse() {
        ...
        // students.sort(myCmp);
        // return students;
        return students.stream().sorted(Comparator.comparing(Student::getProgram)
