...
public class Inventory {
    ...
    public List<String> getBuildables() {
        ...
        if (swords >= 1 && sunStones >= 1 && Game.getActiveZombie() == 0) {
    ...
    public InventoryItem checkBuildCriteria(Player p, boolean remove, boolean forceShield, EntityFactory factory) {
        ...
        if (wood.size() >= 1 && arrows.size() >= 3 && !forceShield) {
            ...
            if (remove) {
                ...
                // SunStone is not removed if used as replacement
            }
        ...
        } else if (swords.size() >= 1 && sunStones.size() >= 1 && Game.getActiveZombie() == 0 && !forceShield) {
            if (remove) {
